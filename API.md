...
Настроен на бэкенде через Spring Security. Разрешённые origins задаются в конфиге (`rpglab.cors.allowed-origins`), локально — `http://localhost:5176`.

| Параметр        | Значение                                                    |
| --------------- | ----------------------------------------------------------- |
| Allowed methods | GET, POST, PUT, PATCH, DELETE, OPTIONS                      |
| Allowed headers | `Authorization`, `Content-Type`                             |
| Exposed headers | `Retry-After` (читать при ответе 429 для обратного отсчёта) |
| Preflight cache | 3600 с                                                      |

`withCredentials` не нужен — аутентификация через заголовок `Authorization: Bearer`, не через cookie.

Для нескольких origins в `.env` указывать через запятую:

```
CORS_ALLOWED_ORIGINS=http://localhost:5176,https://rpglab.app
```
