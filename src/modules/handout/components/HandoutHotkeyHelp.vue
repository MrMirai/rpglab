...
const selectionHints = [
  { tokens: [DRAG], desc: 'выделить / переместить' },
  { tokens: [key('Shift'), key('клик')], desc: 'добавить к выделению' },
  { tokens: [key('2×клик')], desc: 'редактировать текст' },
  { tokens: [key('Esc')], desc: 'снять выделение' },
  { tokens: [key('Delete')], desc: 'удалить выделенное' },
]
...
<template>
  <div :class="['hotkey-help', { 'hotkey-help--open': open }]">
    ...
    <div v-if="open" class="hotkey-help__panel">
      <div
        v-for="(group, gi) in [
          { title: 'Выделение', hints: selectionHints },
          { title: 'Вид', hints: viewHints },
          { title: 'История', hints: historyHints },
        ]"
        :key="gi"
